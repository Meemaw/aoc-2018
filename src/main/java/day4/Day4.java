...
public class Day4 {
    ...
    private static void buildDataMaps(Stream<String> lines, Map<Integer, Long> sleepingMap,
                                      Map<Integer, List<Integer>> guardSleepingMinutes,
                                      Map<String, Long> byMinuteMap) {
        int guardId = 0;
        int startMinute = 0;
        for (Record r : lines.map(Record::new).sorted(Comparator.comparing(r -> r.date)).toArray
                (Record[]::new)) {
            if (r.guardId != 0) {
                guardId = r.guardId;
                ...
                guardSleepingMinutes.put(guardId, Stream.concat(IntStream.range(startMinute, r
                        .minute).boxed(), list.stream()).collect(Collectors.toList()));
                sleepingMap.put(guardId, sleepingMap.getOrDefault(guardId, 0l) + r.minute -
                        startMinute);
    ...
    public static long part1(Stream<String> lines) {
        ...
        int sleeperId = Collections.max(sleepingMap.entrySet(), Comparator.comparingLong(Map
                .Entry::getValue)).getKey();

        return sleeperId * guardSleepingMinutes.get(sleeperId).stream().collect(Collectors
                .groupingBy
                (Function.identity(), Collectors.counting())).entrySet().stream().max(Comparator
                .comparing(Map.Entry::getValue)).get().getKey();
    }
    ...
    public static long part2(Stream<String> lines) {
        ...
        String[] compoundKeySplit = byMinuteMap.entrySet().stream().max(Comparator.comparing(Map
                .Entry::getValue)).get().getKey().split("-");
